...
이 문서는 `/pipet_model` 아래 프로젝트를 다음 담당자가 빠르게 이해하고 실행할 수 있도록 정리한 인수인계 문서입니다.  
정리 범위는 다음과 같습니다.
...
현재 문서는 **현재 저장소에 남아 있는 코드 기준**으로 작성했습니다.
...
이 프로젝트는 마이크로피펫 장비를 대상으로 다음 기능을 통합합니다.
...
이 프로젝트는 **GUI와 추론/비전 worker를 분리**해서 사용합니다.

- GUI는 **시스템 Python** 으로 실행합니다.
- OCR/YOLO/일부 worker 기능은 **conda 환경**에서 실행합니다.
- GUI는 직접 시리얼 포트를 잡고 있습니다.
- GUI는 필요할 때 `conda run -n pipet_env python -m worker.worker ...` 형태로 worker를 호출합니다.

즉, 구조는 다음과 같습니다.

1. 운영자는 시스템 Python으로 GUI를 실행합니다.
2. GUI는 시리얼 연결을 직접 생성합니다.
3. 캡처/YOLO/OCR/run-to-target 판단은 worker 프로세스로 호출합니다.
4. worker는 JSON 결과를 stdout으로 반환합니다.
5. GUI는 경로에 따라 JSON 결과를 해석하거나, 외부 테스트 스크립트 실행 상태를 반영합니다.

### 1.3 왜 이렇게 분리했는가

현재 코드와 README 기준으로 추정되는 설계 이유는 다음과 같습니다.

- PyQt5는 시스템 Python에서 더 안정적으로 동작합니다.
- TensorRT / PyCUDA / Torch / Ultralytics / PaddleOCR 계열은 conda 환경에서 관리하는 편이 충돌이 적습니다.
- GUI와 추론 환경을 분리하면 CUDA/Qt 충돌 위험을 줄일 수 있습니다.
...
- 별도 함수 없이 즉시 실행되는 TensorRT 엔진 점검 스크립트입니다.
...
핵심 컨트롤러입니다. 인수인계 시 가장 먼저 읽어야 하는 파일입니다.
...
- `Controller.start_run_to_target`
  - `single_target_paddleOCR_test.py` 실행
  - 실행 전 GUI 시리얼 포트를 해제
  - 이때는 `subprocess.Popen` 사용
  - stdout/stderr를 별도 스레드로 읽는다

- `Controller._run_to_target_stdout_loop`
  - 테스트 스크립트의 진행 로그와 최종 JSON을 읽는다
  - step/current/error 상태를 GUI에 반영한다
  - 최신 OCR 용량값을 preview에 반영한다
...
GUI에서 가장 많이 호출하는 공용 worker 진입점입니다.
...
YOLO 검출 결과를 후속 OCR이 쓰는 형태로 저장합니다.
...
- `target_panel.py`
  ...
  - 현재 선택 카메라 번호를 OCR / run-to-target에 전달
...
- `pipette_panel.py`
  - 수동 리니어/회전 모터 조작용
  - 자동 보정 실행 중 수동 패널 비활성화

#### `ocr_motor/test/*`

실운영 코드보다는 점검/보조 목적입니다.
...
현재 Python 파일들에는 인수인계형 주석을 추가해 두었습니다.
...
- 현재 문서와 기존 README 기준으로 **GUI는 conda Python이 아니라 시스템 Python으로 실행**합니다.
- PyQt5를 conda에서 실행할 경우 환경 충돌 가능성이 있습니다.
...
즉, worker 전체 기능을 다 쓰려면 아래 수준으로 맞추는 것이 안전합니다.
...
이 파일들이 없거나 경로가 바뀌면 GUI는 떠도 OCR/YOLO worker는 실패합니다.
...
포트/카메라 번호가 다르면 코드 수정 또는 실행 파라미터 조정이 필요합니다.
...
여기서 `python3` 는 **시스템 Python** 을 의미합니다.
...
1. GUI가 현재 목표값과 카메라 번호를 읽습니다.
2. GUI가 자신의 시리얼 포트를 잠시 해제합니다.
3. GUI가 `test/single_target_paddleOCR_test.py` 를 conda 환경에서 실행합니다.
4. 테스트 스크립트가 PaddleOCR 기반으로 현재 용량을 읽고 자체적으로 시리얼 포트를 열어 모터를 구동합니다.
5. GUI는 stdout 로그와 최종 JSON만 읽어 현재 step, volume, 종료 상태를 화면에 반영합니다.
6. 스크립트 종료 후 GUI가 시리얼 포트를 다시 연결하고 기본 액추에이터 설정을 복원합니다.
...
conda run -n pipet_env python -u test/single_target_paddleOCR_test.py --target=2500 --camera=0
```
...
현재 GUI의 `Run To Target` 버튼은 PaddleOCR 기반 테스트 스크립트를 실행하도록 연결되어 있습니다.

- `Read Current Volume (OCR)` 버튼은 기본적으로 TRT worker 경로를 사용합니다.
- `Run To Target` 버튼은 `single_target_paddleOCR_test.py` 경로를 사용합니다.
- 즉, 현재 GUI 안에서도 OCR 경로가 버튼마다 다를 수 있습니다.
- 장비 셋업이나 판독 편차를 점검할 때는 이 차이를 먼저 인지해야 합니다.
...
둘을 하나로 합치려고 하면 PyQt5 / CUDA / TensorRT 쪽에서 문제가 생길 가능성이 있습니다.

### 5.1.2 실제 모터 구동 책임은 GUI에 있다

이 항목은 현재 코드에서 경로별로 구분해서 이해해야 합니다.

- `worker/control_worker.py` 경로는 여전히 제어 판단만 담당합니다.
- 이 경로를 사용할 때는 실제 `VolumeDCActuator.run()` / `stop()` 호출이 `gui/controller.py` 에 있습니다.
- 하지만 현재 GUI의 `Run To Target` 버튼은 이 경로를 직접 쓰지 않습니다.
- 현재 버튼은 `test/single_target_paddleOCR_test.py` 를 실행하며, 이 스크립트가 시리얼 포트를 직접 열고 모터를 구동합니다.

즉, 현재 `Run To Target` 디버깅은 항상 **GUI와 테스트 스크립트를 같이 봐야 합니다**.

### 5.1.3 ROI는 파일로 저장/재사용된다

OCR은 매번 YOLO를 새로 돌리는 구조가 아닙니다.

- YOLO가 `state/rois.json` 저장
- OCR은 그 ROI를 재사용

따라서 카메라 위치나 각도가 바뀌면 ROI를 다시 잡아야 합니다.

---

## 5.2 현재 코드 기준 확인된 주의사항

아래는 인수인계 관점에서 꼭 알아야 하는 현재 상태입니다.

### 5.2.1 `PipettePanel` 의 수동 리니어 축 기능은 일부만 운영 가능 상태입니다

과거 `PipettePanel` 은 아래 메서드를 `Controller` 에 기대하도록 작성되어 있었습니다.
...
현재 정리 기준으로는 다음과 같이 보는 것이 안전합니다.

- `선형 액추에이터 목표 위치 이동(linear_move)` 경로는 현재 연결되어 있습니다.
...
- 위치 입력 이동 버튼은 `Controller.linear_move()` 를 통해 동작합니다.
- 상/하 토글 버튼은 예외를 내지 않고 안내 메시지를 띄우도록 막아 두었습니다.
- 자동 보정 실행 중에는 수동 패널 전체가 비활성화됩니다.
...
### 5.2.2 `YoloPanel` 의 ROI 오버레이 표시는 현재 코드 기준으로 보완되어 있습니다

이전에는 `YoloPanel.show_fixed_rois()` 가 `VideoPanel.show_pixmap()` 을 호출하지만
대상 메서드가 없어 예외 가능성이 있었습니다.

현재는 아래와 같이 정리되어 있습니다.

- `VideoPanel.show_pixmap()` 메서드를 추가해 메모리 상 pixmap 표시가 가능합니다.
- `YoloPanel` 은 worker 결과에서 `annotated_path` 를 우선 사용하고, 없으면 `frame_path` 를 보조로 사용합니다.
- YOLO 결과 이미지 위에 정규화 ROI를 그린 뒤 preview 패널에 바로 반영합니다.

즉, 인수인계 시 이 구간은 **예전 문제는 있었지만 현재 코드에서는 보완된 상태**로 보면 됩니다.

### 5.2.3 `TargetPanel.update_camera_frame()` 는 현재 미사용 가능성 높음

`TargetPanel` 내부에 `self.camera_label.setPixmap(...)` 호출이 있으나,  
현재 클래스에서 `camera_label` 을 만드는 코드가 없습니다.

현재는 속성이 없을 때 바로 반환하도록 가드를 넣어 두었습니다.

즉, 이 메서드는 **당장 예외를 내지는 않지만 예전 코드 잔재일 가능성이 높습니다**.

추가로 현재 `TargetPanel` 의 실제 OCR / Run To Target 동작은 `VideoPanel` 에서 선택한 카메라 번호를 함께 사용하도록 보완되어 있습니다.

따라서 인수인계 시에는 다음처럼 이해하면 됩니다.

1. `update_camera_frame()` 는 사실상 레거시 메서드입니다.
2. 실제 카메라 선택 연동은 `TargetPanel._camera_index()` 경로를 보면 됩니다.
3. OCR과 Run To Target은 현재 preview와 같은 카메라 번호를 사용합니다.

### 5.2.4 `test_ocr_only.py` 의 모델 경로는 절대경로 기반

`worker/test_ocr_only.py` 에는 다음 절대경로가 남아 있습니다.

```python
OCR_PT_PATH = "/home/sixr/Desktop/pipet_model/ocr_motor/best_efficientnet_origin.pt"
```

현재 환경과 다를 수 있으므로 그대로는 동작하지 않을 가능성이 높습니다.

### 5.2.5 README 구조 설명은 현재 코드와 일부 불일치

기존 `ocr_motor/README.md` 안 구조 설명에는 다음처럼 현재와 다른 이름이 있습니다.

- `motor_test_panel.py`
- `ocr_worker.py`
- `motor_worker.py`

실제 인수인계는 README보다 **현재 디렉터리와 본 문서**를 기준으로 하는 것이 안전합니다.
...
이 프로젝트는 **시스템 Python GUI + conda worker + 시리얼 제어를 공유하는 혼합 구조**입니다.  
인수인계 시에는 먼저 `Controller` 중심 흐름을 이해하고, 그다음 OCR/YOLO/state 파일/테스트 스크립트/시리얼 계층 순서로 보는 것이 가장 빠릅니다.
